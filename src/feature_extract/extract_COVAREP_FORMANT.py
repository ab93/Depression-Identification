...
header_f = ["video", "question","question_number", "starttime", "endtime", 'formant1_mean', 'formant2_mean', 'formant3_mean',
...
def get_feature_vector(f,instance,item,writer1,writer2,startTime,endTime,split_seconds):
    if split_seconds == 0:
        ...
        if questionType_DND[instance[0]] == 'D' or questionType_PN[instance[0]] == 'P':
            writer1.writerow(vector)
        else:
            writer2.writerow(vector)
    else:
        time_split = startTime
        while time_split <= endTime: 
            
            startFrame = time_split * 100
            endFrame = (time_split + (split_seconds*100)) * 100
            ...
            if questionType_DND[instance[0]] == 'D' or questionType_PN[instance[0]] == 'P':
                ...
                writer2.writerow(vector)
            time_split = time_split + split_seconds     
...
def readFORMANT_DND(split_seconds):
    print 'FORMANT DND'
    groupByQuestion = {}
    if split_seconds == 0:
    ...
    for item in groupByQuestion:
        ...
        for instance in groupByQuestion[item]:
            ...
            get_feature_vector(f,instance,item,dWriter,ndWriter,startTime,endTime,split_seconds)
...
def readFORMANT_PN(split_seconds):
    print 'FORMANT PN'
    groupByQuestion = {}
    gender=pd.Series.from_csv('data/misc/gender.csv').to_dict()
    if split_seconds == 0:
    ...
    for item in groupByQuestion:
        ...
        for instance in groupByQuestion[item]:
            ...
            get_feature_vector(f,instance,item,pWriter,nWriter,startTime,endTime,split_seconds)
...
def readCOVAREP_DND(split_seconds):
    print 'COVAREP DND'
    groupByQuestion = {}
    if split_seconds == 0:    
...
def readCOVAREP_PN(split_seconds):
    print 'COVAREP PN'
    groupByQuestion = {}
    gender=pd.Series.from_csv('data/misc/gender.csv').to_dict()
    if split_seconds == 0:
